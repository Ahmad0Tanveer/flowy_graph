...
class LinePainter extends CustomPainter {
  ...
  final FlowyNodeController controller;
  final BuildContext context;
  final FlowyBackgroundBase background;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (controller.startPointConnection != null &&
      ...
      Offset? pos = getPortPosition(
        controller.outNodeName!,
        controller.outPortName!,
      );
      _drawConnection(
        canvas,
        size,
  ...
  Offset? getPortPosition(String nodeName, String portName) {
    ...
    if (port is InPort) {
      ...
      return Offset(
        pos.dx + iconSize.width * 3 / 4,
        pos.dy + iconSize.height / 2,
      );
  ...
  Offset getObjectPosition(GlobalKey key) {
    ...
    return stackRenderBox.localToGlobal(Offset.zero) -
        controller.stackPos! +
        Offset(
          controller.horizontalScrollController.offset,
  ...
  void drawConnections(Canvas canvas, Size size) {
    for (Connection conn in controller.connections) {
      Offset? startPoint = getPortPosition(
        conn.outNode.name,
        conn.outPort.name,
      );
      ...
      conn.connectionPath.drawConnection(
        canvas,
        size,
